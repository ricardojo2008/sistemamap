...
$userito= get_current_user().",<br>Ip Cliente: $ipcliente <BR> Host :".gethostname().", <BR>IP :".gethostbynamel(gethostname())[0].",<BR> Sistema :".php_uname();
	...
	$query = "update recambiorosario set serial='$serial',parnew='$parnew',estatus='$estatus',fecha_cambio=now() where id='$id'";
...
function preparar_mensaje($id,$userito){
	...
	//buscamos los datos del equipo que se esta cambiando en nuestra base de datos...
		$query = "select id,sucursal,direccion,empleado,numa,cargo, par, ip,observacion,modelo,ram,cpu,hd,estatus,parnew,serial FROM recambiorosario where id='$id'";
